...
public class Matrix{
	...
	public void setMatrix(String str){
		int rowSize = rowCount(str);
		int colSize = numCount(str)/rowSize;
		
		if(rowSize == matrixField.length & colSize == matrixField[0].length){
			
			for(int i = 0; i < rowSize; i ++){
				if(i != (rowSize-1)){
					int [] numArr = strToNumArr(str.substring(0,str.indexOf(";")));
					str = str.substring(str.indexOf(";")+1,str.length());
					for(int j = 0; j < colSize; j ++){
						matrixField[i][j] = numArr[j];
					}
				}else{
					int [] numArr = strToNumArr(str);
					for(int j = 0; j < colSize; j ++){
						matrixField[i][j] = numArr[j];
					}
				}
